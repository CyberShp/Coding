...
```json
{
  ...
  "remote": {
    ...
    "ingest_url": ""                                             // Agent 推送地址，如 http://192.168.1.100:8001/api/ingest
  }
}
```
...
| 问题 | 排查方法 |
|------|---------|
| 端口被占用 | `lsof -i :8001` 或 `netstat -tlnp \| grep 8001`，终止占用进程或修改端口 |
...
| 服务 | 端口 | 说明 |
|------|------|------|
| Web 后端 API | 8001 | FastAPI 服务 |
| 前端开发服务器 | 5174 | Vite 开发模式 |
| SSH 连接 | 22 | 连接存储阵列 |
| WebSocket | 8001 | 与后端同端口，路径 /ws/alerts |
...
2. **Web 后端修改后**：
   - 重启后端：`python -m uvicorn backend.main:app --reload --port 8001`
   - 点击阵列详情 → 刷新，观察活跃问题面板
   - 检查 API 响应：`curl http://localhost:8001/api/alerts/recent?limit=5 | python -m json.tool`
